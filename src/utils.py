...
def nms_darkflow(Labels, iou_threshold=0.5):
    SelectedLabels = []
...
#
def nms_bike_darkflow_target(ocr, iou_threshold=0.3, target_characters=np.inf):
    #
    ...
    for p in ocr:
        if p["topleft"]["y"] + p["bottomright"]["y"] < 80:
            ...
            bottom.append(p)
    top = nms_darkflow_target(top, 0.3, 3)  # three characters top
    bottom = nms_darkflow_target(bottom, 0.3, 4)  # four characters bottom
    return top + bottom
...
#
def nms_darkflow_range(
    Labels,
    iou_threshold=0.25,
    min_threshold=0.4,
    min_characters=0,
    max_characters=np.inf,
):
    SelectedLabels = []
    #
    ...
    for label in Labels:
        ...
        #
        if (
            len(SelectedLabels) > min_characters
            and SelectedLabels[-1]["confidence"] < min_threshold
        ):
...
#
def nms_darkflow_target(Labels, iou_threshold=0.25, target_characters=np.inf):
    SelectedLabels = []
    #
    #  Sorts detection in descending order of confidence
    #
...
def adjust_pts(pts, result):
    tl = np.array(
        [result["topleft"]["x"], result["topleft"]["y"]], dtype=float
    ).reshape(2, 1)
    wh = np.array(
        [
...
def print_digits(Ilp, ocr_list, font=1):
    ...
    for ocr in ocr_list:
        ...
        cv2.putText(
            Ilp, ocr["label"], (tlx, tly), cv2.FONT_HERSHEY_SIMPLEX, font, dig_color, 2
        )
