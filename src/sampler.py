...
def labels2output_map(labelist, lpptslist, dim, stride, alpha=0.75):
    ...
    #
    side = ((float(dim0) + 40.0) / 2.0) / stride  # 7.75 when dim = 208 and stride = 16
    ...
    for i in range(0, len(labelist)):
        ...
        #
        if (
            max(lppts[0,]) - min(lppts[0,]) > 0.01
            and max(lppts[1,]) - min(lppts[1,]) > 0.01
        ):
...
def project(I, T, pts, dim):
    ...
    ptsret = ptsh[:2]
    ptsret = ptsret / dim
    Iroi = cv2.warpPerspective(I, T, (dim, dim), borderValue=0.0, flags=cv2.INTER_CUBIC)
    return Iroi, ptsret
...
def project_all(I, T, ptslist, dim, bgimages=bgimages):
    ...
    Iroi = cv2.warpPerspective(
        I, T, (dim, dim), borderValue=(0.5, 0.5, 0.5), flags=cv2.INTER_CUBIC
    )
    if len(bgimages) > 0:
        bgimage = bgimages[int(np.random.rand() * len(bgimages))]
        bgimage = random_crop(bgimage, dim, dim)
        bw = np.ones(I.shape)
        bw = cv2.warpPerspective(
            bw, T, (dim, dim), borderValue=(0, 0, 0), flags=cv2.INTER_LINEAR
        )
...
def randomblur(img):
    ...
    sig = abs(np.random.normal(0, 0.1)) * maxblur
...
def augment_sample(
    I, shapelist, dim, maxangle=2 * np.array([65.0, 65.0, 55.0]), maxsum=140
):
    #
    ...
    if pts[0][1] - pts[0][0] > 0.002:  ## if not fake plate
        for i in range(len(ptslist)):
            #
