...
def draw_losangle(I, pts, color=(1.0, 1.0, 1.0), thickness=1):
...
def write2img(
    Img, label, strg, txt_color=(0, 0, 0), bg_color=(255, 255, 255), font_size=1
):
    ...
    if (tl_corner < 0.0).any():
        delta = 0.0 - np.minimum(tl_corner, 0.0)
    elif (br_corner > 1.0).any():
        delta = 1.0 - np.maximum(br_corner, 1.0)
    else:
        delta = 0.0
